...
public class SQSQueueResponse {
	...
	public AmazonSQS createServiceQueue() {
		...
		try {
			...
			Policy policy = new Policy().withId(Constants.ARN_SQS + Constants.RESP_QUEUE + Constants.SQSDefaultPolicy)
					.withStatements(
							new Statement(Effect.Allow).withId("Sid1494892084085").withPrincipals(Principal.AllUsers)
									...
									.withResources(new Resource(Constants.ARN_SQS + Constants.RESP_QUEUE)));
			...
			CreateQueueRequest createQueueRequest = new CreateQueueRequest(Constants.RESP_QUEUE)
					.withAttributes(attributes).addAttributesEntry("ReceiveMessageWaitTimeSeconds", "10");
	...
	public void sendMsgtoQueue(String sendMsg, AmazonSQS sqs) {
		...
		counterReq++;
		// Send a message
		System.out.println("Sending a message to SQSQueueResponse for Service Operation.\n");
